...
//! The default constructor sets the level to 1
LevelMapBuilder::LevelMapBuilder()
{
	level = 1;
}

//! The constructor sets the level to the given level
//! @param m_level the level to be set
LevelMapBuilder::LevelMapBuilder(int m_level)
{
	level = m_level;
}
...
//! Builds the monsters within the map and sets 
//! their level to the level of the map
//! and stores them in the m_monsters variable
void LevelMapBuilder::buildMonsters()
{
	Monster leveledMonster = Monster(level);
...
void LevelMapBuilder::buildMap(string fileName)
{
	...
	for (int i = 0; i < rows; i++)
	{
		...
		for (int j = 0; j < columns; j++)
		{
			if (m_map->get(i, j) == "t")
			{
				...
				{
					Item item = itemGen.generateItem(level);
					//m_treasure->at(treasureCounter).addItem(item);
...
void LevelMapBuilder::printTreasure(vector<vector<Item>> *treasureList)
{
...
void LevelMapBuilder::printMonsters(vector<Monster>* monsters)
{
