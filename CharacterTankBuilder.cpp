//!	 @file CharacterTankBuilder.cpp
//!  @brief Concrete builder implementation of the tank fighter
//!
//!  This class builds a bully fighter which has stats tailored for survival
...
void CharacterTankBuilder::buildCharacter(int level)
{
	log = Logger();
	...
	log.LogCharacter("Setting tank character ability scores");
	// Set stats according to order of importance
	log.LogCharacter("Setting character consitution: " + to_string(scores[0]));
	c_character->setCon(scores[0]);
	log.LogCharacter("Setting character dexterity: " + to_string(scores[1]));
	c_character->setDex(scores[1]);
